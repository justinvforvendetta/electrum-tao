...
from electrum_tao import SimpleConfig, Wallet, WalletStorage, format_satoshis
from electrum_tao.bitcoin import is_address
...
def pay_to(recipient, amount, label):
    ...
    droid.dialogCreateSpinnerProgress("Electrum-TAO", "signing transaction...")
    droid.dialogShow()
        ...
        if event["name"] == "click":
            ...
            if id=="buttonPay":
                ...
                if not is_address(recipient):
                    modal_dialog('Error','Invalid Tao address')
                    continue
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in TAO). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
...
class ElectrumGui:
    ...
    def __init__(self, config, _network):
        ...
        if not storage.file_exists:
            ...
            droid.dialogCreateSpinnerProgress("Electrum-TAO", msg)
            droid.dialogShow()
